...
const ForecastDays: React.FC<ForecastDaysProps> = ({ value }) => {
  ...
  return (
    <div className="flex">
      <div
        className={`flex flex-col justify-center items-center border-[#314159]`}
      >
        <div className="my-4 lg:mr-0 flex">
          <div className="flex flex-col justify-center items-center mr-2">
            ...
            >
              {Math.floor(value.day.avgtemp_f)}°F
            </h1>
          </div>
          <div className="flex flex-col justify-center mr-2">
            ...
            >
              Max Temp: {Math.floor(value.day.maxtemp_f)}
            </h1>
            <h1
              className={`${am.includes(currentStatus) ? "text-[#c4cad3]" : "text-[#c4cad3]"
                } text-[9px] lg:text-xs`}
            >
              Min Temp: {Math.floor(value.day.mintemp_f)}
            </h1>
            <h1
              className={`${am.includes(currentStatus) ? "text-[#c4cad3]" : "text-[#c4cad3]"
                } text-[9px] lg:text-xs`}
            >
              Chances of rain: {Math.floor(value.day.daily_chance_of_rain)}%
            </h1>
          </div>
        </div>
      </div>
